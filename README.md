...
**1. 官方源码的物理损失在实现上退化，且与论文正文直接冲突。**
论文方法节写明「用有限差分近似 dθ/dt」（*We approximate dθ̂ᵢ/dt using finite differences across consecutive time windows*），
而官方代码是 `dtheta_dt = torch.zeros_like(phases)`（`code_3:249`）。损失因此退化为一个把耦合强度压向零的正则项。

**2. 论文自报的物理损失效应量，小于它自己的折间噪声。**
论文原文为 `Physics constraints (+0.9%) improve generalization despite small loss weight`，
而 HP-GNN 的折间标准差是 ±2.1%（表 5）。本复现的对照实验测得 **+0.36%**（轮间波动 0.124）——
与论文的 +0.9% 一致。**所以结论不是「没能复现出这个效果」，而是这个量级本就落在噪声内。**
另一个佐证：物理损失把耦合强度 K 从 0.87 压到 0.22，而论文声称学到的 K = 0.71±0.14
「恰好落在奇美拉区间 [0.4–0.9] 内」——正确实现这个损失，反而把 K 推出了该区间。
